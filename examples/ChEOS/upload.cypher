...
MATCH p=()-[r]->() detach delete r

// NODES

// Substance
//LOAD CSV WITH HEADERS FROM "file:///ETF_auradb_substance_nodes.csv"
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_substance_nodes.csv"
AS nodeRecord
WITH nodeRecord WHERE nodeRecord.`DTXSID` IS NOT NULL
...
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_site_nodes.csv"
AS nodeRecord
WITH nodeRecord WHERE nodeRecord.`station_name_n` IS NOT NULL
...
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_species_nodes.csv"
AS nodeRecord
WITH nodeRecord WHERE nodeRecord.`species` IS NOT NULL
...
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_is_driver_relation.csv" AS relRecord
...
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_measured_at_relation.csv" AS relRecord
MATCH (source: `Substance` { `DTXSID`: relRecord.`DTXSID` })
MATCH (target: `Site` { `name`: relRecord.`station_name_n` })
MERGE (source)-[r: `MEASURED_AT`]->(target)
SET r.`key` = toInteger(relRecord.`key`)
...
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_tested_for_toxicity_relation.csv" AS relRecord
MATCH (source: `Substance` { `DTXSID`: relRecord.`DTXSID` })
MATCH (target: `Species` { `name`: relRecord.`species` })
MERGE (source)-[r: `TESTED_FOR_TOXICITY`]->(target)
SET r.`key` = toInteger(relRecord.`key`)
...
LOAD CSV WITH HEADERS FROM "https://schoradore.de/data/ETF_auradb_summarized_impact_on_relation.csv" AS relRecord
MATCH (source: `Site` { `name`: relRecord.`station_name_n` })
MATCH (target: `Species` { `name`: relRecord.`species` })
MERGE (source)-[r: `SUMMARIZED_IMPACT_ON`]->(target)
SET r.`key` = toInteger(relRecord.`key`)
SET r.`time_point` = datetime(relRecord.`time_point`)
SET r.`year` = toInteger(relRecord.`year`)
SET r.`quarter` = toInteger(relRecord.`quarter`)
